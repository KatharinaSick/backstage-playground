...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [notificationsPlugin],
    });

    await request(server).get('/api/notifications/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/notifications/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get(`/api/notifications/todos/${createdTodoItem.id}`)
